...
export class PigsComponent implements OnInit {
  ...
  */
  initForms() {
    ...
    // Form Editor
    this.formEditPig = this.formBuilder.group({
      _id: [''],
      ...
      number_of_females: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(10)]],
      number_of_males: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(10)]],
      total_pigs: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(10)]],
    });
  }
  ...
  chargeForm() {
    ...
    this.formEditPig.patchValue({
      _id: this.dataSelect.pigs[0]._id,
      number_females_0_to_6_months: this.dataSelect.pigs[0].number_females_0_to_6_months,
      number_females_6_to_12_months: this.dataSelect.pigs[0].number_females_6_to_12_months,
      number_females_12_to_24_months: this.dataSelect.pigs[0].number_females_12_to_24_months,
      number_females_older_than_24_months: this.dataSelect.pigs[0].number_females_older_than_24_months,
      number_males_0_to_6_months: this.dataSelect.pigs[0].number_males_0_to_6_months,
      number_males_6_to_12_months: this.dataSelect.pigs[0].number_males_6_to_12_months,
      number_males_12_to_24_months: this.dataSelect.pigs[0].number_males_12_to_24_months,
      number_males_older_than_24_months: this.dataSelect.pigs[0].number_males_older_than_24_months,
      number_of_females: this.dataSelect.pigs[0].number_of_females,
      number_of_males: this.dataSelect.pigs[0].number_of_males,
      total_pigs: this.dataSelect.pigs[0].total_pigs,
    });
  }
  ...
  getErrorMessageEdit(field: string): string {
    let message = '';
    if (this.formEditPig.get(field)!.hasError('required')) {
      message = 'Este campo es requerido';
    } else if (this.formEditPig.get(field)!.hasError('minlength')) {
      const minLength = this.formEditPig
        .get(field)!.errors?.minlength.requiredLength;
      message = `Este campo debe tener un minimo de ${minLength} caracteres`;
    } else if (this.formEditPig.get(field)!.hasError('maxlength')) {
      const maxLength = this.formEditPig
        .get(field)!.errors?.maxlength.requiredLength;
      message = `Este campo debe tener un maximo de ${maxLength} caracteres`;
    } else if (this.formEditPig.get(field)!.hasError('pattern')) {
      message = 'Este campo es invalido';
    } else if (this.formEditPig.get(field)!.hasError('min')) {
      const min = this.formEditPig.get(field)!.errors?.min.min;
      message = `Este campo debe ser mayor a ${min}`;
    } else if (this.formEditPig.get(field)!.hasError('max')) {
      const max = this.formEditPig.get(field)!.errors?.max.max;
  ...
  invalidField(field: string): boolean {
    if (this.formEditPig.get(field)!.invalid && this.formSubmitted) {
      return true;
    }
    return false;
  }
  ...
  saveChange() {
    ...
    if (this.formEditPig.invalid) {
      console.log('ERROR WE');
      console.log(this.formEditPig);
    } else {
      this.arrayInv.push(this.formEditPig.value);
      this.formEditData.patchValue({
        pigs: this.arrayInv,
      });
