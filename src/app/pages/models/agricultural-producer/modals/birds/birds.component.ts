...
export class BirdsComponent implements OnInit {
  ...
  */
  initForms() {
    ...
    // Form Editor
    this.formEditBird = this.formBuilder.group({
      _id: [''],
      dual_purpose_number: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(10)]],
      fattening_number: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(10)]],
      number_of_layers: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(10)]],
      number_of_roosters: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(10)]],
    });
  }
  ...
  chargeForm() {
    ...
    this.formEditBird.patchValue({
      _id: this.dataSelect.birds[0]._id,
      dual_purpose_number: this.dataSelect.birds[0].dual_purpose_number,
      fattening_number: this.dataSelect.birds[0].fattening_number,
      number_of_layers: this.dataSelect.birds[0].number_of_layers,
      number_of_roosters: this.dataSelect.birds[0].number_of_roosters,
    });
  }
  ...
  getErrorMessageEdit(field: string): string {
    let message = '';
    if (this.formEditBird.get(field)!.hasError('required')) {
      message = 'Este campo es requerido';
    } else if (this.formEditBird.get(field)!.hasError('minlength')) {
      const minLength = this.formEditBird
        .get(field)!.errors?.minlength.requiredLength;
      message = `Este campo debe tener un minimo de ${minLength} caracteres`;
    } else if (this.formEditBird.get(field)!.hasError('maxlength')) {
      const maxLength = this.formEditBird
        .get(field)!.errors?.maxlength.requiredLength;
      message = `Este campo debe tener un maximo de ${maxLength} caracteres`;
    } else if (this.formEditBird.get(field)!.hasError('pattern')) {
      message = 'Este campo es invalido';
    } else if (this.formEditBird.get(field)!.hasError('min')) {
      const min = this.formEditBird.get(field)!.errors?.min.min;
      message = `Este campo debe ser mayor a ${min}`;
    } else if (this.formEditBird.get(field)!.hasError('max')) {
      const max = this.formEditBird.get(field)!.errors?.max.max;
  ...
  invalidField(field: string): boolean {
    if (this.formEditBird.get(field)!.invalid && this.formSubmitted) {
      return true;
    }
    return false;
  }
  ...
  saveChange() {
    ...
    if (this.formEditBird.invalid) {
      console.log('ERROR WE');
      console.log(this.formEditBird);
    } else {
      this.arrayInv.push(this.formEditBird.value);
      this.formEditData.patchValue({
        birds: this.arrayInv,
      });
